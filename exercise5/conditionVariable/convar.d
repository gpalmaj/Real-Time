...
class Resource(T) {
    ...
    this(){
      ...
      mtx.lock();
      queue.insert(id, priority);          // join the queue
      while (queue.front() != id) {        // am I first?
          cond.wait();                     // no → release mutex + sleep + re-lock mutex
      }
  ...
  void deallocate(T v) {
      mtx.lock();
      value = v;
      queue.popFront();                    // remove myself from front
      cond.notifyAll();                    // wake ALL sleeping threads
      mtx.unlock();
  }
}
