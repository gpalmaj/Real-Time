...
All packages depend downward into `config` and `models`.
...
```bash
# Start the elevator server first (or the simulator, if testing at home)
...
go run main.go 0   # connects to elevator simulator on port 15657
go run main.go 1   # connects to elevator simulator on port 15658
go run main.go 2   # connects to elevator simulator on port 15659
```
